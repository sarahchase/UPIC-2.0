...
!dir$ attributes align : 64 :: ppart, pparti
...
! nvpy/nvpz = number of processors in y/z
      call mpfcomp3(nvp,nx,ny,nz,nvpy,nvpz,ierr)
      if (ierr /= 0) then
         if (kstrt==1) then
            write (*,*) 'mpfcomp3 error: nvp,nvpy,nvpz=', nvp, nvpy, nvpz
         endif
         call PPEXIT(); stop
      endif
!
...
! write ion velocity-space diagnostic output: updates nvirec
                  if (nvft < 4) then
                     if (kstrt==1) then
                        call dafwritefv3(fvmi,fvi,fei,wk,tdiag,iuvi,    &
     &nvirec)
                     endif
                  endif
               endif
...
! updates pparti and wki, and possibly ncl, ihole, irc
     ...
     &,wki,tpush,nx,ny,nz,mx,my,mz,mx1,myp1,ipbc,popt,relativity,plist, &
     &irc)
         wki = wki*rmass
!
